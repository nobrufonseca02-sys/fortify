...
export interface CoverflowCarouselProps {
  ...
  /**
   * How the slide image fills its (square) card. "cover" suits full-bleed
   * photography — the original use case. "contain" suits a centred mark on
   * its own backing (a brand logo) that must never be cropped.
   */
  imageFit?: "cover" | "contain";
  /**
   * Fires with the index of the card that settled in the centre. Lets a page
   * drive a detail panel from the carousel without duplicating its state —
   * the carousel stays the single source of truth for what is selected.
   */
...
/** Deslocamento, em px, a partir do qual o gesto deixa de ser clique. */
const DRAG_SLOP = 8;
...
export function CoverflowCarousel({
  ...
  /** Pixels travelled since pointerdown. A card click only counts as a click
   *  below the threshold — otherwise ending a drag on a card would jump the
   *  carousel to wherever the finger happened to lift. */
  ...
  const onPointerMove = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    // Passou de um tremido: vira arrasto e aí sim captura o ponteiro, para
    // o gesto continuar mesmo se o cursor sair do frame.
    if (movedRef.current > DRAG_SLOP && !event.currentTarget.hasPointerCapture(event.pointerId)) {
  ...
  const endDrag = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    // Clique parado é escolha, e quem trata é o onClick do card. Aqui só
    // interessa não assentar por cima dela.
  ...
  // Efeito, e não chamada dentro do handler, porque `selected` também muda
  // no meio do arrasto e no assentamento — um só lugar cobre os três caminhos.
  React.useEffect(() => {
          ...
          >
            {slides.map((slide, index) => (
              <div
                ...
                data-cf-index={index}
                onClick={() => {
                  // Um arrasto termina com um clique no card sob o dedo — só
                  // conta como escolha se o ponteiro praticamente não andou.
                  if (movedRef.current > DRAG_SLOP) return;
      ...
      {showCaption && active?.title && (
        ...
        >
          ...
          {active.meta && active.meta.length > 0 && (
            <dl className="mt-10 w-full max-w-xs text-[12px]">
              {active.meta.map((row) => (
                <div key={row.label} className="flex items-baseline gap-3 py-[5px]">
                  <dt className="shrink-0 text-muted-foreground">{row.label}</dt>
                  <dd className="min-w-0 flex-1 text-right font-medium leading-snug text-foreground">{row.value}</dd>
                </div>
              ))}
            </dl>
          )}
